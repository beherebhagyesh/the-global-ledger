...
const ACHIEVEMENTS: Achievement[] = [
    ...
    { id: 'diamond_hands', title: 'Diamond Hands', description: 'Hold Crypto while having > $5000 cash.', icon: 'Gem', xpReward: 800, rarity: 'LEGENDARY' },
    { id: 'the_wolf', title: 'The Wolf', description: 'Close a deal with "Aggressive" tactics.', icon: 'Briefcase', xpReward: 500, rarity: 'RARE' },
    ...
    useEffect(() => {
        ACHIEVEMENTS.forEach(ach => {
            if (!player.achievements.includes(ach.id)) {
                let unlocked = false;
                if (ach.id === 'first_steps' && !['INTRO', 'MENTOR_SELECTION', 'CLASS_SELECTION', 'LEVEL_1'].includes(player.level)) unlocked = true;
                ...
                if (ach.id === 'stress_free' && player.stress < 10 && !['LEVEL_1', 'INTRO'].includes(player.level)) unlocked = true;
                ...
                if (ach.id === 'debt_free' && !['LEVEL_1', 'INTRO'].includes(player.level)) unlocked = true;
