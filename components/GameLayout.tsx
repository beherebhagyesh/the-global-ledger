...
export const GameLayout: React.FC<GameLayoutProps> = ({ children }) => {
  const { player, unlockSkill, jumpToLevel, purchaseHQ } = useGame();
  ...
  return (
    <div className="h-screen bg-slate-950 text-slate-200 flex overflow-hidden relative">
      ...
      {isMobile && isSidebarOpen && (
        <div className="fixed inset-0 bg-black/80 z-40 backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)} />
      )}

      <Inventory items={player.inventory} isOpen={isInventoryOpen} onClose={() => setIsInventoryOpen(false)} />
      {isCalculatorOpen && <RateCalculator onClose={() => setIsCalculatorOpen(false)} />}
      {isSkillTreeOpen && <SkillTree player={player} onClose={() => setIsSkillTreeOpen(false)} onUnlock={unlockSkill} />}
      {isHQBuilderOpen && <HQBuilder player={player} onClose={() => setIsHQBuilderOpen(false)} onPurchase={purchaseHQ} />}

      <aside className={`fixed md:relative z-50 h-full bg-slate-900 border-r border-slate-800 flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-0 md:w-20 -translate-x-full md:translate-x-0'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800 shrink-0">
          {isSidebarOpen ? (
            ...
            <div className="w-full flex justify-center"><Activity className="text-emerald-400 shrink-0" /></div>
          )}
          {!isMobile && isSidebarOpen && (
            <button onClick={toggleSidebar} className="text-slate-500 hover:text-white"><ChevronLeft size={20} /></button>
          )}
        </div>
        {!isMobile && !isSidebarOpen && (
          <button onClick={toggleSidebar} className="w-full py-4 flex justify-center text-slate-500 hover:text-white border-b border-slate-800"><ChevronRight size={20} /></button>
        )}
        {isMobile && (<button onClick={() => setIsSidebarOpen(false)} className="absolute top-4 right-4 text-slate-400"><X /></button>)}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-6 px-0 scrollbar-thin scrollbar-thumb-slate-700">
              ...
              {isSidebarOpen ? (
                <>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs bg-emerald-900 text-emerald-300 px-2 py-0.5 rounded shrink-0">Lvl {Object.values(GameLevel).indexOf(player.level)}</span>
                  </div>
                  {player.characterClass !== 'NONE' && (<div className="text-xs text-slate-400 mb-2 font-mono uppercase tracking-tight">Class: {player.characterClass.replace('_', ' ')}</div>)}
                  ...
                  <div className="flex items-center justify-between bg-slate-950 rounded p-2 border border-slate-700 mb-2" title="CIBIL/FICO Score">
                    ...
                    <span className={`font-mono font-bold text-sm ${player.creditScore >= 750 ? 'text-green-400' : player.creditScore >= 650 ? 'text-yellow-400' : 'text-red-400'}`}>{player.creditScore}</span>
                  </div>
                  <div className="flex items-center justify-between bg-slate-950 rounded p-2 border border-slate-700 mb-2" title="Stress Level">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div className={`h-full transition-all duration-500 ${player.stress > 70 ? 'bg-red-500' : player.stress > 40 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${player.stress}%` }}></div>
                      </div>
                  ...
                  {player.loans && player.loans.length > 0 && (
                    <div className="bg-red-900/10 border border-red-900/30 rounded p-2 mb-2">
                      ...
                      <div className="text-xs text-slate-300 flex justify-between"><span>Balance:</span><span>-${player.loans.reduce((acc, l) => acc + l.remainingBalance, 0).toLocaleString()}</span></div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between bg-slate-950 rounded p-2 border border-slate-700 mb-2"><span className="text-xs text-slate-500 uppercase flex items-center gap-1"><Star size={10} /> Reputation</span><span className="text-yellow-400 font-mono font-bold text-sm">{player.reputation}/100</span></div>
                  <div className="w-full bg-slate-700 h-2 rounded-full overflow-hidden mb-1"><div className="bg-emerald-500 h-full transition-all duration-1000 ease-out" style={{ width: `${Math.min(100, (player.xp / 10000) * 100)}%` }}></div></div>
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="w-1 h-8 bg-slate-700 rounded-full overflow-hidden"><div className="bg-emerald-500 w-full transition-all" style={{ height: `${Math.min(100, (player.xp / 10000) * 100)}%` }}></div></div>
                </div>
              )}
            </div>
            {isSidebarOpen && (
              <div className="space-y-2 mt-4">
                <button onClick={() => setIsInventoryOpen(true)} className="w-full flex items-center gap-2 p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 transition-colors"><Backpack size={16} /> <span className="text-xs font-bold uppercase">Inventory</span></button>
                <button onClick={() => setIsSkillTreeOpen(true)} className="w-full flex items-center gap-2 p-2 bg-slate-800 hover:bg-slate-700 text-indigo-300 rounded border border-slate-700 transition-colors"><Zap size={16} /> <span className="text-xs font-bold uppercase">Skills</span></button>
                <button onClick={() => setIsHQBuilderOpen(true)} className="w-full flex items-center gap-2 p-2 bg-slate-800 hover:bg-slate-700 text-blue-300 rounded border border-slate-700 transition-colors"><LayoutGrid size={16} /> <span className="text-xs font-bold uppercase">HQ Upgrades</span></button>
                <button onClick={() => setIsCalculatorOpen(true)} className="w-full flex items-center gap-2 p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 transition-colors"><Calculator size={16} /> <span className="text-xs font-bold uppercase">Rate Calc</span></button>
              </div>
            )}
            {isSidebarOpen && player.achievements.length > 0 && (
              <div className="mt-4 space-y-2 animate-in slide-in-from-left">
                ...
                {player.achievements.slice(-3).map((ach, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs text-yellow-500 bg-yellow-500/10 p-2 rounded border border-yellow-500/20"><Trophy size={12} /><span className="truncate capitalize">{ach.replace('_', ' ')}</span></div>
                ))}
              </div>
            )}
          </div>
          <div className="space-y-1 px-3">
            ...
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level1)} current={player.level} level={GameLevel.Level1} label="Foundations" icon={Brain} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level2)} current={player.level} level={GameLevel.Level2} label="Corp Engine" icon={Activity} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level3)} current={player.level} level={GameLevel.Level3} label="Tax Bridge" icon={Target} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level4)} current={player.level} level={GameLevel.Level4} label="Investing" icon={TrendingUp} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level5)} current={player.level} level={GameLevel.Level5} label="Fundraising" icon={Building2} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.Level6)} current={player.level} level={GameLevel.Level6} label="Commodities" icon={Container} />
            <LevelItem collapsed={!isSidebarOpen} onClick={() => jumpToLevel(GameLevel.FinalBoss)} current={player.level} level={GameLevel.FinalBoss} label="Client Pitch" icon={Trophy} />
          </div>
        </div>
      </aside>
      <main className="flex-1 h-full overflow-y-auto relative bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black pt-16 md:pt-0">
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto relative z-0 min-h-full flex flex-col justify-center">{children}</div>
      </main>
    </div>
  );
};
